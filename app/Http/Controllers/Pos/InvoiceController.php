...
class InvoiceController extends Controller
{
    ...
    public function InvoiceAdd()
    {
        ...
        if ($invoice_data == null) {
         ...
         $date = date('Y-m-d'); 
        return view('backend.invoice.invoice_add',compact('date','invoice_no','category','customer'));
    }

    public function InvoiceStore(Request $request)
    {
        if ($request->category_id == null) {
            return back()->with('select_item','You did not Select any item!');
        }else {
            if ($request->paid_amount > $request->estimated_amount) {
                return back()->with('paid_amount','Sorry Paid Amount is Maximum than total price!');
            }else {
                $invoice = new Invoice;
        ...
        return redirect()->route('invoice.pending.list')->with('alldata','Invoice data Inserted Successfully');
    }
    ...
    public function InvoiceDelete($id)
    {
       ...
       return redirect()->back()->with('deletedata','Invoice data Deleted Successfully');
    }
    ...
    public function ApprovalStore(Request $request,$id)
    {
        foreach ($request->selling_qty as $key => $value) {
           ...
           if ( $product->quantity < $request->selling_qty[$key]) {
                return redirect()->back()->with('maximumapprove','Sorry you approve maximum value');
           }
           ...
           return redirect()->route('invoice.pending.list')->with('approve','Invoice Approve Successfully');
        }
    }
